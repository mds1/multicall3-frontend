...
function getKindInfo(kind: 'success' | 'warning' | 'error' | 'info') {
  if (kind === 'success')
    return { icon: CheckCircleIcon, iconColor: 'text-green-500 dark:text-green-400' };
  if (kind === 'warning')
    return { icon: ExclamationCircleIcon, iconColor: 'text-yellow-500 dark:text-yellow-400' };
  if (kind === 'error') return { icon: XCircleIcon, iconColor: 'text-red-600 dark:text-red-500' };
  return { icon: InformationCircleIcon, iconColor: 'text-blue-500 dark:text-blue-400' };
}
              ...
              <div className='p-4'>
                <div className='flex items-start'>
                  ...
                  <div className='ml-3 w-0 flex-1 pt-0.5'>
                    <p className={`text-sm font-medium ${titleTextColor}`}>{title}</p>
                    <p className={`mt-1 text-sm ${descriptionTextColor}`}>{description}</p>
                  </div>
